	...
	bool editable)
{
	...
	if(editable)
		((int32*)m_column_types.Items())[column_index] |= CLVColFlagEditable;
}
...
void CLVEasyItem::SetColumnUserTextContent(int column_index, bool truncate, bool right_justify, bool editable)
{
	PrepListsForSet(column_index);
	if(truncate)
...
void CLVEasyItem::Update(BView *owner, const BFont *font)
{
	...
	text_offset = ceil(FontAttributes.ascent) + floor((Height()-FontHeight)/2.0);
}
...
const char* CLVEasyItem::EditInitialText(int32 column_index) const
{
	...
	if(type == CLVColStaticText || type == CLVColTruncateText)
		text = (const char*)m_column_content.ItemAt(column_index);
	else if(type == CLVColTruncateUserText || type == CLVColUserText)
		text = GetUserText(column_index,-1);
	else
		assert(!"Text column");
	return text;
...
BPoint CLVEasyItem::ColumnTextDrawPoint(int32 column_index, const ColumnListView* owner) const
{
	BRect item_column_rect = ItemColumnFrame(column_index,owner);
	return BPoint(item_column_rect.left+2.0,item_column_rect.top+text_offset);
}
