...
void ColumnListView::ColumnsChanged()
{
	...
	if(fEditTextView)
		fEditTextView->DoneEditing(false);
...
void ColumnListView::SetSortKey(int32 ColumnIndex, bool doSort)
{
	...
	if(ColumnIndex >= 0)
	{
		assert(ColumnIndex < fColumnList.CountItems());
...
void ColumnListView::MouseDown(BPoint where)
{
	...
	fWatchingForEdit = false;
	if(fEditTextView)
		fEditTextView->DoneEditing(true);
	if(item_index >= 0)
	{
...
void ColumnListView::MouseUp(BPoint where)
{
	if(fWatchingForDrag)
	{
		if(fSoleSelectionAtRelease)
			Select(fNoKeyMouseDownItemIndex,false);
		int32 column_index = ColumnIndex(fNoKeyMouseDownItemIndex,where.x);
		if(column_index >= 0)
		{
			CLVListItem* item = (CLVListItem*)ItemAt(fNoKeyMouseDownItemIndex);
			if(item->Editable(column_index))
			{
				//User just released an editable column: wait for the double-click time or watch for
				//the mouse to move out of the column rectangle
				fWatchingForEdit = true;
				fEditItem = item;
				fEditColumn = ColumnAt(column_index);
				fEditWatchRect = item->ItemColumnFrame(column_index,this);
				fStartEditWatchTime = system_time();
			}
		}
	}
...
void ColumnListView::Pulse()
{
	...
	{
		bigtime_t min_time;
		if(get_click_speed(&min_time) == B_NO_ERROR && system_time() - fStartEditWatchTime >= min_time)
		{
			InitiateEdit();
			fWatchingForEdit = false;
		}
...
void ColumnListView::MouseMoved(BPoint where, uint32 code, const BMessage *message)
{
	...
	else if(fWatchingForEdit && (code == B_EXITED_VIEW || !fEditWatchRect.Contains(where)))
	{
		//Mouse left the edit area: initiate editing
		InitiateEdit();
		fWatchingForEdit = false;
	}
}
...
void ColumnListView::InitiateEdit()
{
	int32 column_index = IndexOfColumn(fEditColumn);
	if(IndexOf(fEditItem) == -1 || column_index == -1)
		return;
	BFont font;
	fEditItem->GetEditingFont(column_index,&font,this);
	const char* edit_text = fEditItem->EditInitialText(column_index);
	BPoint text_point = fEditItem->ColumnTextDrawPoint(column_index,this);
	BRect text_rect = StringBoundingBox(edit_text,&font,true);
	text_rect.OffsetBy(text_point.x,text_point.y);
	if(text_rect.left > text_rect.right)
		text_rect.left = text_rect.right;
	text_rect.right = fEditItem->ItemColumnFrame(column_index,this).right;
	assert(!fEditTextView);
	fEditTextView = new CLVEditTextView(text_rect,edit_text,
		fSelectedItemColorWindowActive);
	AddChild(fEditTextView);
}
	...
	if(fHierarchical)
	{
		...
		else
		{
			...
			if(Visible)
			{
				...
				{
					if(fEditTextView && IndexOf(fEditItem) >= PreviousItemPos+1)
						fEditTextView->MoveTo(fEditTextView->Frame().LeftTop()+BPoint(0,item->Height()));
				}
				return Result;
			}
			return true;
...
bool ColumnListView::AddList(BList* newItems, int32 fullListIndex)
{
	...
	{
		status_t result = BListView::AddList(newItems,fullListIndex);
		if(result == B_NO_ERROR && fEditTextView && IndexOf(fEditItem) >= fullListIndex)
		{
			float delta_y = 0;
			int32 num_items = newItems->CountItems();
			for(int32 i=0; i<num_items; i++)
				delta_y += ((BListItem*)newItems->ItemAt(i))->Height();
			fEditTextView->MoveTo(fEditTextView->Frame().LeftTop()+BPoint(0,delta_y));
		}
		return result;
	}
}
...
bool ColumnListView::EfficientAddListHierarchical(BList* newItems, int32 fullListIndex, bool expanded)
{
	...
	if(expanded)
	{
		...
		if(fullListIndex > 0)
		{
			int32 previous_showing_full_list_index = fullListIndex-1;
			bool done = false;
			while(previous_showing_full_list_index > -1 && !done)
			{
				index = IndexOf((CLVListItem*)fFullItemList.ItemAt(previous_showing_full_list_index));
				if(index == -1)
					previous_showing_full_list_index--;
				else
					done = true;
			}
			index++;
		...
		if(!BListView::AddList(newItems,index))
			return false;
		if(fEditTextView && IndexOf(fEditItem) >= index)
		{
			float delta_y = 0;
			int32 num_items = newItems->CountItems();
			for(int32 i=0; i<num_items; i++)
...
bool ColumnListView::RemoveItem(BListItem* a_item)
{
	...
	if(fEditTextView)
	{
		int32 remove_index = IndexOf(a_item);
		int32 edit_index = IndexOf(fEditItem);
		if(remove_index != -1)
		{
			if(remove_index < edit_index)
				fEditTextView->MoveTo(fEditTextView->Frame().LeftTop()+BPoint(0,0-a_item->Height()));
			else if(remove_index == edit_index)
				fEditTextView->DoneEditing(false);
		}
	}
	if(fHierarchical)
...
BListItem* ColumnListView::RemoveItem(int32 fullListIndex)
{
	if(fEditTextView)
	{
		int32 remove_index = fullListIndex;
		if(fHierarchical)
			remove_index = IndexOf((CLVListItem*)fFullItemList.ItemAt(fullListIndex));
		int32 edit_index = IndexOf(fEditItem);
		if(remove_index != -1)
		{
			if(remove_index < edit_index)
			{
				fEditTextView->MoveTo(fEditTextView->Frame().LeftTop()+BPoint(0,0-
					ItemAt(remove_index)->Height()));
			}
			else if(remove_index == edit_index)
				fEditTextView->DoneEditing(false);
		}
...
bool ColumnListView::RemoveItems(int32 fullListIndex, int32 count)
{
	...
	if(fHierarchical)
	{
		...
		int32 edit_index = 0;
		if(fEditTextView)
			edit_index = IndexOf(fEditItem);
		float delta_y = 0;
		bool remove_edit_view = (fEditTextView != NULL);
		while(DisplayItemsToRemove > 0)
		{
			if(fEditTextView)
			{
				if(edit_index > FirstDisplayItemToRemove)
					delta_y -= ItemAt(FirstDisplayItemToRemove)->Height();
				else if(edit_index == FirstDisplayItemToRemove)
					fEditTextView->DoneEditing(false);
			}
		...
		if(remove_edit_view)
			fEditTextView->DoneEditing(false);
		else if(fEditTextView && delta_y < 0)
			fEditTextView->MoveTo(fEditTextView->Frame().LeftTop()+BPoint(0,delta_y));
	...
	else
	{
		...
		{
			int32 edit_index = IndexOf(fEditItem);
			if(fullListIndex <= edit_index)
			{
				if(fullListIndex+count-1 < edit_index)
				{
					float delta_y = 0;
					for(int32 i=fullListIndex; i<fullListIndex+count; i++)
						delta_y -= ItemAt(i)->Height();
					fEditTextView->MoveTo(fEditTextView->Frame().LeftTop()+BPoint(0,delta_y));
				}
				else
					fEditTextView->DoneEditing(false);
...
void ColumnListView::SortItems()
{
	...
	while((SelectedItemIndex = CurrentSelection(SelectionIndex)) >= 0)
	{
		SelectedItems.AddItem(ItemAt(SelectedItemIndex));
		SelectionIndex++;
	}
	if(!fHierarchical)
	{
	...
	{
		int32 SelectedItemIndex = IndexOf((BListItem*)SelectedItems.ItemAt(i));
		if(SelectedItemIndex >= 0)
			Select(SelectedItemIndex,true);
	}
}
...
int32 ColumnListView::ColumnIndex(int32 item_index, float x)
{
	int32 NumberOfColumns = fColumnDisplayList.CountItems();
	float PushMax = 0;
	CLVColumn* ThisColumn;
	float column_right = -1;
	CLVListItem* item = (CLVListItem*)ItemAt(item_index);
	float ExpanderDelta = item->OutlineLevel() * 20.0;
	//Figure out what the limit is for expanders pushing other columns
	for(int32 Counter = 0; Counter < NumberOfColumns; Counter++)
	{
		ThisColumn = (CLVColumn*)fColumnDisplayList.ItemAt(Counter);
	...
	for(int32 Counter = 0; Counter < NumberOfColumns; Counter++)
	{
		...
		if(!ThisColumn->IsShown())
			continue;
		column_right = ThisColumn->fColumnEnd;
		if((ThisColumn->fFlags & CLV_EXPANDER) || ThisColumn->fPushedByExpander)
		{
			column_right += ExpanderDelta;
			if(column_right > PushMax)
				column_right = PushMax;
		}
		if(column_right >= x)
			return IndexOfColumn((CLVColumn*)fColumnDisplayList.ItemAt(Counter));
	}
	return -1;
}


void ColumnListView::EditCompleted(const char* text)
{
	if(fEditItem)
		fEditItem->EditCompleted(IndexOfColumn(fEditColumn),text,this);
	fEditItem = NULL;
	fEditColumn = NULL;
	fEditTextView = NULL;
}
...
void ColumnListView::EditAborted()
{
	fEditItem = NULL;
	fEditColumn = NULL;
	fEditTextView = NULL;
...
void ColumnListView::ShowAsInactiveWhenNotKeyboardFocus()
{
	fShowAsInactiveWhenNotKeyboardFocus = true;
	Invalidate();
}


rgb_color ColumnListView::ItemSelectColor() const
{
	if(!fWindowActive)
		return fSelectedItemColorWindowInactive;
	if(fShowAsInactiveWhenNotKeyboardFocus && ((!Window()) || (Window()->CurrentFocus()!=(BView*)this &&
		!(fEditTextView && Window()->CurrentFocus() == fEditTextView))))
	{
		return fSelectedItemColorWindowInactive;
	}
	return fSelectedItemColorWindowActive;
}


void ColumnListView::MakeFocus(bool focus)
{
	BListView::MakeFocus(focus);
	if(fShowAsInactiveWhenNotKeyboardFocus && fWindowActive && CurrentSelection(0) != -1)
		Invalidate();
}


//******************************************************************************************************
//**** CLVContainerView CLASS DEFINITION
//******************************************************************************************************
...
//******************************************************************************************************
//**** CLVEditTextView CLASS DEFINITION
//******************************************************************************************************
CLVEditTextView::CLVEditTextView(BRect text_rect, const char* edit_text, rgb_color background)
: BTextView(BRect(text_rect.left-1,text_rect.top,text_rect.right,text_rect.bottom),NULL,
	text_rect.OffsetToCopy(0,0),B_FOLLOW_LEFT|B_FOLLOW_TOP,B_WILL_DRAW)
{
	...
	if(edit_text)
	{
		m_original_text = Strdup_new(edit_text);
		SetText(edit_text);
	}
	else
...
CLVEditTextView::~CLVEditTextView()
{
	if(m_original_text)
		delete[] m_original_text;
}


void CLVEditTextView::AttachedToWindow()
{
	MakeFocus(true);
	const char* text = Text();
	if(text)
		Select(0,strlen(text));
}


void CLVEditTextView::MakeFocus(bool focus)
{
...
void CLVEditTextView::WindowActivated(bool active)
{
...
void CLVEditTextView::KeyDown(const char* bytes, int32 num_bytes)
{
	if(num_bytes == 1 && bytes[0] == B_ESCAPE)
		DoneEditing(false);
	else if(num_bytes == 1 && bytes[0] == B_ENTER)
		DoneEditing(true);
	else
...
void CLVEditTextView::DoneEditing(bool ok_to_activate)
{
	if(m_removing)
		return;
	m_removing = true;
	if(((!m_original_text) || m_original_text[0] == 0) && ((!Text()) || Text()[0] == 0))
		ok_to_activate = false;
	if(m_original_text && Text() && strcmp(m_original_text,Text())==0)
		ok_to_activate = false;
	if(ok_to_activate)
		((ColumnListView*)Parent())->EditCompleted(Text());
	else
		((ColumnListView*)Parent())->EditAborted();
